...
struct UsageStoreHighestUsageTests {
    ...
    func `selects highest usage among enabled providers`() {
        let settings = testSettingsStore(suiteName: "UsageStoreHighestUsageTests-selects")
    ...
    func `skips fully used providers`() {
        let settings = testSettingsStore(suiteName: "UsageStoreHighestUsageTests-skips")
        settings.refreshFrequency = .manual
        settings.statusChecksEnabled = false
